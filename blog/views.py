...
def get_blog_list_common_date(request, blogs_all_list):
    ...
    # 只显示当前页的周围4个,不显示所有标签
    page_range = [x for x in range(
        int(page_num) - 1, int(page_num) + 2) if 0 < x <= paginator.num_pages]
...
def blog_with_readnums(request):
    blogs_all_list = Blog.objects.all()
    blog_dict = {}
    for blog in blogs_all_list:
        nums = blog.get_read_nums
        blog_dict[nums] = blog
    new_blog_list = dict(sorted(blog_dict.items(), key=lambda x: x, reverse=True)).values()
    context = get_blog_list_common_date(request, list(new_blog_list))
    return render(request, 'blog/blog_list.html', context)
...
def blog_detail(request, pk):
    ...
    guide_blogs = Blog.objects.filter(Q(blog_type=blog_type) & ~Q(id=pk))[:10]

    # 获取类型统计
    blog_type_list = BlogType.objects.annotate(blog_count=Count('blog'))
    context['blog_types'] = blog_type_list
    context['guideblogs'] = guide_blogs
    read_cookie_key = read_statistics_once_read(request, blog)
    # 自定义计数规则
    context['blog'] = blog
    context['previous_blog'] = Blog.objects.filter(
        create_time__gt=blog.create_time).last()  # 上一篇博客 因为排序是倒过来
    context['next_blog'] = Blog.objects.filter(
        create_time__lt=blog.create_time).first()
    # 获取对应博客的评论内容
    blog_content_type = ContentType.objects.get_for_model(blog)
    comment = Comment.objects.filter(
        content_type=blog_content_type, object_id=pk, parent=None)
